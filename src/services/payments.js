...
document.addEventListener("DOMContentLoaded", () => {
  ...
  async function generateInvoice(amountSats, memo = "Motorcycle Game Payment") {
    try {
  ...
  async function payWithQR(amountSats, memo = "Motorcycle Game Payment") {
    try {
      const resp = await fetch("/api/create-invoice", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  async function waitForPayment(paymentHash, timeout = 5 * 60 * 1000) {
  ...
  async function makePayment(amountSats, memo = "Motorcycle Game Payment") {
    try {
